...
function ReverseProxy(opts, callback){

  opts = opts || {};

  opts.port = null === typeof opts.port ? 8080 : opts.port;
  opts.hostname = null === typeof opts.hostname ? 'localhost' : opts.hostname;

  if(!(this instanceof ReverseProxy)){
    return new ReverseProxy(opts, callback);
  }
  ...
  // If callback was provided, bind it to "listen" events.
  if( 'function' === typeof callback ) {
    proxy.on( 'http:listen', callback );
    proxy.on( 'https:listen', callback );
  }
  ...
  if(opts.ssl){
    ...
    httpsServer.listen(opts.ssl.port, opts.host, function() {
      self.emit( 'https:listen', null, this );
    });
  }
  ...
  server.listen(opts.port, opts.host, function() {
    self.emit( 'ready', null, this );
    self.emit( 'http:listen', null, this );
  });
...
//
// EventEmitter Prototype mix-in
//
ReverseProxy.prototype = Object.create( EventEmitter.prototype, {
...
ReverseProxy.prototype.register = function(src, target, opts){
  ...
  // Allow route configuration to be passed as an object.
  if( 'object' === typeof src ) {
    target = src.target;
    opts = src.opts || src.options
    src = src.src || src.host || src.hostname;
  }
